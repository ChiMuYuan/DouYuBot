...
class keepalive(Thread):
    ...
    # def sendmeesage(self, Str):
    #     code = [0xb1, 0x02, 0x00, 0x00]
    #     end = 0
    #     Str = "type@=keeplive/tick@=" + str(int(time.time())) + "/"
    #     lenth = [calcMessageLength(Str), 0, 0, 0]
    #     Str = bytearray(Str, "ascii")
    #     tot = bytearray()
    #     for i in lenth:
    #         tot.append(i)
    #     for i in lenth:
...
def showdm(data):
    ...
    # elif spid == 12:
    #     gid = data.find(b'/gn@=')
    #     drid = data.find(b'/drid@=')
    #     gs = data.find(b'/gs@=')
    #     ggg = bytearray()
    #     for i in range(drid + 7, gs):
    #         ggg.append(data[i])
    #     if ggg.decode("utf8") == "74751":
    #         ggg = bytearray()
    #         for i in range(gid + 5, drid):
    #             ggg.append(data[i])
    #         print(data)
    #         print("大礼物 " + ggg.decode("utf8"))
...
SocketSDM.connect((socket.gethostbyname("119.90.49.90"), 8099))
...
loginInfo = "type@=loginreq/username@=/ct@=0/password@=/roomid@=74751/devid@=" + uuiddd + "/rt@=" + timenow + "/vk@=" + vk + "/ver@=20150929/aver@=2017073111/ltkid@=/biz@=1/stk@=/"
...
while True:
    ssssss = input()
    SocketSDM.sendall(sendmeesage("type@=chatmessage/receiver@=0/content@=" + ssssss + "/scope@=/col@=0/pid@=/p2p@=0/nc@=0/rev@=0/ifs@=0/"))
    print(SocketSDM.recv(10240).replace(b'@A', b'@').replace(b'@S', b'/'))
